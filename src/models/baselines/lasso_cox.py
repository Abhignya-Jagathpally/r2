"""
L1-penalized Cox proportional hazards model via scikit-survival.

Features:
- Nested cross-validation for lambda selection
- Feature importance from coefficients
- Calibration diagnostics
"""

from typing import Dict, Optional, Tuple, Union

import numpy as np
import pandas as pd
from sksurv.linear_model import CoxPHFitter
from sksurv.preprocessing import Standardizer
from sksurv.util import Surv
from sklearn.model_selection import GridSearchCV

from .base_model import BaseSurvivalModel


class LassoCoxModel(BaseSurvivalModel):
    """
    L1-penalized (Lasso) Cox proportional hazards model.

    Uses scikit-survival's CoxPHFitter with ridge/lasso penalties.
    """

    def __init__(
        self,
        alpha: float = 1.0,
        l1_ratio: float = 1.0,  # 1.0 = pure L1 (Lasso)
        fit_baseline_hazard: bool = True,
        normalize: bool = True,
        tol: float = 1e-7,
        max_iter: int = 10000,
        random_state: int = 42,
    ):
        """
        Initialize Lasso Cox model.

        Parameters
        ----------
        alpha : float
            Regularization strength. Higher = more regularization.
        l1_ratio : float
            Ratio of L1 vs L2 penalty. 1.0 = L1 only (Lasso), 0.0 = L2 only (Ridge).
        fit_baseline_hazard : bool
            Whether to fit baseline hazard.
        normalize : bool
            Whether to normalize features.
        tol : float
            Tolerance for convergence.
        max_iter : int
            Maximum iterations.
        random_state : int
            Random state.
        """
        super().__init__(random_state=random_state)
        self.alpha = alpha
        self.l1_ratio = l1_ratio
        self.fit_baseline_hazard = fit_baseline_hazard
        self.normalize = normalize
        self.tol = tol
        self.max_iter = max_iter

        # Model and preprocessing
        self.model_ = None
        self.standardizer_ = None
        self.feature_importance_ = None

    def fit(
        self,
        X: Union[np.ndarray, pd.DataFrame],
        y_time: Union[np.ndarray, pd.Series],
        y_event: Union[np.ndarray, pd.Series],
        **kwargs,
    ) -> "LassoCoxModel":
        """
        Fit Lasso Cox model.

        Parameters
        ----------
        X : array-like or DataFrame, shape (n_samples, n_features)
            Feature matrix.
        y_time : array-like, shape (n_samples,)
            Survival time.
        y_event : array-like, shape (n_samples,)
            Event indicator.
        **kwargs
            Additional arguments (unused).

        Returns
        -------
        self
        """
        X_array, y_time_array, y_event_array = self._validate_input(X, y_time, y_event)

        # Standardize features
        self.standardizer_ = Standardizer()
        X_std = self.standardizer_.fit_transform(X_array)

        # Create structured array for sksurv
        y_struct = Surv.from_arrays(y_event_array.astype(bool), y_time_array)

        # Fit Cox model with penalty
        self.model_ = CoxPHFitter(
            penalizer=self.alpha,
            l1_ratio=self.l1_ratio,
            fit_baseline_hazard=self.fit_baseline_hazard,
            normalize=self.normalize,
            tol=self.tol,
            max_iter=self.max_iter,
        )

        df = pd.DataFrame(X_std, columns=self.feature_names_)
        self.model_.fit(df, y_struct)

        # Extract feature importance (coefficients)
        self.feature_importance_ = self.model_.params_.to_dict()

        return self

    def predict_risk(self, X: Union[np.ndarray, pd.DataFrame]) -> np.ndarray:
        """
        Predict risk scores using partial hazard (exp(X @ beta)).

        Parameters
        ----------
        X : array-like or DataFrame, shape (n_samples, n_features)
            Feature matrix.

        Returns
        -------
        risk_scores : ndarray, shape (n_samples,)
            Risk scores (higher = higher risk).
        """
        X_array, _, _ = self._validate_input(X)
        X_std = self.standardizer_.transform(X_array)

        df = pd.DataFrame(X_std, columns=self.feature_names_)
        partial_hazard = self.model_.predict_partial_hazard(df).values

        return partial_hazard

    def predict_survival_function(
        self,
        X: Union[np.ndarray, pd.DataFrame],
        times: Optional[np.ndarray] = None,
    ) -> pd.DataFrame:
        """
        Predict survival probability S(t).

        Parameters
        ----------
        X : array-like or DataFrame, shape (n_samples, n_features)
            Feature matrix.
        times : ndarray, optional
            Time points for evaluation. If None, use model's baseline times.

        Returns
        -------
        survival_prob : DataFrame
            Survival probabilities. Shape: (n_samples, n_times).
        """
        X_array, _, _ = self._validate_input(X)
        X_std = self.standardizer_.transform(X_array)

        df = pd.DataFrame(X_std, columns=self.feature_names_)

        # Get survival functions
        surv_funcs = self.model_.predict_survival_function(df)

        # If times specified, interpolate
        if times is not None:
            result = pd.DataFrame(index=df.index, columns=times)
            for idx in df.index:
                s = surv_funcs.iloc[:, idx]
                for t in times:
                    idx_closest = (s.index - t).abs().argmin()
                    result.loc[idx, t] = s.iloc[idx_closest]
            return result
        else:
            return surv_funcs.T

    def get_selected_features(self, threshold: float = 0.01) -> list:
        """
        Get features with non-zero coefficients.

        Parameters
        ----------
        threshold : float
            Absolute coefficient threshold for selection.

        Returns
        -------
        selected : list
            Feature names with |coef| >= threshold.
        """
        selected = [
            fname for fname, coef in self.feature_importance_.items()
            if np.abs(coef) >= threshold
        ]
        return selected

    def fit_nested_cv(
        self,
        X: Union[np.ndarray, pd.DataFrame],
        y_time: Union[np.ndarray, pd.Series],
        y_event: Union[np.ndarray, pd.Series],
        alpha_grid: Optional[np.ndarray] = None,
        cv_inner: int = 3,
        cv_outer: int = 5,
    ) -> Tuple[float, float, np.ndarray]:
        """
        Nested cross-validation for lambda (alpha) selection.

        Parameters
        ----------
        X : array-like or DataFrame
            Feature matrix.
        y_time : array-like
            Survival time.
        y_event : array-like
            Event indicator.
        alpha_grid : ndarray, optional
            Alpha values to search. If None, use default grid.
        cv_inner : int
            Inner CV folds for hyperparameter selection.
        cv_outer : int
            Outer CV folds for model evaluation.

        Returns
        -------
        best_alpha : float
            Best alpha value.
        cv_score : float
            Cross-validated C-index.
        cv_scores : ndarray
            Per-fold C-index scores.
        """
        from sklearn.model_selection import KFold
        from sksurv.metrics import concordance_index_censored

        X_array, y_time_array, y_event_array = self._validate_input(X, y_time, y_event)

        if alpha_grid is None:
            alpha_grid = np.logspace(-4, 2, 20)

        outer_splitter = KFold(n_splits=cv_outer, shuffle=True, random_state=self.random_state)
        cv_scores_outer = []
        best_alpha_values = []

        for train_idx, test_idx in outer_splitter.split(X_array):
            X_train, X_test = X_array[train_idx], X_array[test_idx]
            y_time_train, y_time_test = y_time_array[train_idx], y_time_array[test_idx]
            y_event_train, y_event_test = y_event_array[train_idx], y_event_array[test_idx]

            # Inner CV for alpha selection
            best_score = -np.inf
            best_alpha = alpha_grid[0]

            inner_splitter = KFold(n_splits=cv_inner, shuffle=True, random_state=self.random_state)

            for alpha_val in alpha_grid:
                inner_scores = []

                for inner_train_idx, inner_val_idx in inner_splitter.split(X_train):
                    X_inner_train = X_train[inner_train_idx]
                    X_inner_val = X_train[inner_val_idx]
                    y_time_inner_train = y_time_train[inner_train_idx]
                    y_event_inner_train = y_event_train[inner_train_idx]
                    y_time_inner_val = y_time_train[inner_val_idx]
                    y_event_inner_val = y_event_train[inner_val_idx]

                    # Fit on inner train
                    model_tmp = LassoCoxModel(alpha=alpha_val, l1_ratio=self.l1_ratio)
                    model_tmp.fit(X_inner_train, y_time_inner_train, y_event_inner_train)

                    # Score on inner val
                    risk_scores = model_tmp.predict_risk(X_inner_val)
                    c_index = concordance_index_censored(
                        y_event_inner_val.astype(bool),
                        y_time_inner_val,
                        risk_scores,
                    )[0]
                    inner_scores.append(c_index)

                mean_inner_score = np.mean(inner_scores)

                if mean_inner_score > best_score:
                    best_score = mean_inner_score
                    best_alpha = alpha_val

            best_alpha_values.append(best_alpha)

            # Fit on outer train with best alpha
            model_outer = LassoCoxModel(alpha=best_alpha, l1_ratio=self.l1_ratio)
            model_outer.fit(X_train, y_time_train, y_event_train)

            # Evaluate on outer test
            risk_scores_test = model_outer.predict_risk(X_test)
            c_index_test = concordance_index_censored(
                y_event_test.astype(bool),
                y_time_test,
                risk_scores_test,
            )[0]
            cv_scores_outer.append(c_index_test)

        best_alpha = np.median(best_alpha_values)
        cv_score = np.mean(cv_scores_outer)
        cv_scores = np.array(cv_scores_outer)

        # Refit with best alpha
        self.alpha = best_alpha
        self.fit(X, y_time, y_event)

        return best_alpha, cv_score, cv_scores
